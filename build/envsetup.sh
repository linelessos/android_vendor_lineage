...
function lineageremote()
{
    ...
    local LINEAGE_USER=$(git config --get review.review.lineless.org.username)
    if [ -z "$LINEAGE_USER" ]
    then
        git remote add lineage ssh://review.lineless.org:29418/$PFX$PROJECT
    else
        git remote add lineage ssh://$LINEAGE_USER@review.lineless.org:29418/$PFX$PROJECT
    fi
...
function lineagegerrit() {
    ...
    local user=`git config --get review.review.lineless.org.username`
...
function lineagerebase() {
    ...
    git fetch "http://review.lineless.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
