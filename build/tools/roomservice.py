...
if not depsonly:
    print("Device %s not found. Attempting to retrieve device repository from linelessos Github (http://github.com/linelessos)." % device)
...
if not depsonly:
    githubreq = urllib.request.Request("https://api.github.com/search/repositories?q=%s+user:linelessos+in:name+fork:true" % device)
    add_auth(githubreq)
    try:
...
print("Repository for %s not found in the linelessos Github repository list. If this is in error, you may need to manually add it to your local_manifests/roomservice.xml." % device)
